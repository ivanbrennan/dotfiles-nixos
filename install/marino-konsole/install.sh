...
__cleanup() {
  ...
  unset -v MARINO_KONSOLE_SCRIPT_OPTS SCHEME_FILE VERBOSE LOCAL_INSTALL MARINO_KONSOLE_VERSION
...
__local_install() {
  ...
  cp -b $SCHEME_FILE $LOCAL_INSTALL_DIR
  if [ $? -eq 0 ]; then
...
MARINO_KONSOLE_SCRIPT_OPTS=`getopt -o vhs: --long verbose,help,schemefile: -n 'install.sh' -- "$@"`
SCHEME_FILE=$DOTFILES/install/marino-konsole/src/marino.colorscheme
VERBOSE=false
LOCAL_INSTALL_DIR=~/.local/share/konsole
MARINO_KONSOLE_VERSION=0.1.0

eval set -- "$MARINO_KONSOLE_SCRIPT_OPTS"
while true; do
  case "$1" in
